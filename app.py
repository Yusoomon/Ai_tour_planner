        ...
        with st.container(border=True):
            ...
            with st.form("onboarding_form", border=False):
                ...
                submit_btn = st.form_submit_button(
                    "Start Planning", type="primary", width="stretch"
                )
...
def render_map_and_routes(col_right):
    ...
    with col_right:
        ...
        with st.container(border=True):
            if not routes:
                st.info(
                    "Fill out the preferences on the left and click **'Generate Top 5 Route Options'**!"
                )
                default_spots = [
                ...
                if map_locations:
                    m = folium.Map(
                        location=[map_locations[0]["lat"], map_locations[0]["lng"]],
                        zoom_start=13,
                    )
                    ...
                    for idx, loc in enumerate(map_locations, 1):
                        ...
                        if remaining_count == 0 and current_day_idx < len(count) - 1:
                            if len(current_day_coords) > 0:
                                day_polylines.append(
                                    {
                                        "coords": current_day_coords,
                                        "color": current_color,
                                    }
                                )
                            current_day_coords = []
                            ...
                            current_color = day_colors[
                                current_day_idx % len(day_colors)
                            ]
                    ...
                    if len(current_day_coords) > 1:
                        day_polylines.append(
                            {"coords": current_day_coords, "color": current_color}
                        )
                        ...
                        for p_idx, place in enumerate(day_info.get("places", []), 1):
                            with st.container(border=True):
                                ...
                                with info_col:
                                    ...
                                    st.markdown(
                                        f"<div style='font-size: 15px; margin-top: -20px'>- Duration: {place.get('estimated_time')} &nbsp;|&nbsp; 📞 {contact_text}</div>",
                                    ...
                                    if overview_text:
                                        with st.expander("Overview", expanded=False):
                                            st.markdown(
                                                f"<div style='font-size: 0.85rem; color: #333;'>{overview_text}</div>",
                                                unsafe_allow_html=True,
                                            )

                                st.markdown(
                                    f"<div style='font-size: 14px; color: gray; margin-top: -10px; margin-bottom: -10px;'>"
                                    f"{place.get('address')} &nbsp;|&nbsp; <i>Estimated Time: {place.get('estimated_real_time')}</i>"
                                    f"</div>",
                                    unsafe_allow_html=True,
                                )
...
def main():
    ...
    if st.session_state["step"] == 1:
        ...
        st.caption(
            f"Profile: {profile['nationality']}, {profile['age']} / Destination: {profile['destination']}"
        )
        st.divider()
